...
class Grid {
   ...
   public:
    ...
    std::function<bool(int, int)> compAny() {
        return [](int r, int c) { return true; };
    }
    ...
    // O(RC)
    // @returns The first (lexigraphically smallest) coordinate which satisfies `isValid`, otherwise
    // returns {-1, -1}
    ...
    // O(1)
    // @returns If the provided coordinate is valid, set its value to `val`, otherwise silently do
    // nothing
    ...
    // @note This is only garunteed to be correct after initPrefix() is called, and the grid is not
    // modified after that
    // @note The rectangular region will be capped so it stays within the grid
   ...
   private:
    ...
    // `dirs`, taking a maximum of `maxD` steps
    // @param `dirs` the valid directions to move to adjacent nodes
   ...
   public:
    ...
    // O(RC)
    // Determines whether a region of coordinates which satisfy `isValid` are commpletely surrounded
    // coordinates which satisfy `isBoundary`
    bool isSurroundedBy(
            ...
            for (int c = 0; c < C; ++c) {
                if (isValid(r, c)) {
                    for (std::pair<int, int> adj : getAdjCoords(r, c, dirs)) {
                        if (!isValid(adj.first, adj.second) && !isBoundary(adj.first, adj.second))
                            return false;
                    }
                }
            }
    ...
    // O(RC)
    // Finds the shortest path from (r1, c1) to (r2, c2) inclusive, where each coordinate on the
    // path satisfies `isValid`
    // @param `dirs` the valid directions to move to adjacent nodes
