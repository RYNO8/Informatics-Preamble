...
// error: non-type template parameters of class type only available with ‘-std=c++2a’ or
// ‘-std=gnu++2a’ template<std::enable_if_t<!is_persistent::value, bool> = true> thankfully c++
// metaprogramming has a true_type template<typename is_persistent_ = is_persistent,
// std::enable_if_t<!is_persistent_::value, bool> = true> template<typename is_persistent_ =
// is_persistent, std::enable_if_t<is_persistent_::value, bool> = true> oop turns out I'm not gonna
// do constexpr stuff

// I should be left and right identity of Combine
// Combine should have `T operator()(T, T)` which is associative property
// CombineAgg should have `operator()(T x, unsigned ll n)` which is defined as adding x to itself n
// times (n > 0)
...
class Segtree : public Range<ll> {
   ...
   private:
    enum LazyMode {
        ...
        // relative, corresponds to `data := Update(data, v)` operations (i.e. augmenting
        // operations)
        AUGMENT,
