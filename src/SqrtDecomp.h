...
namespace DS {
// O(Q sqrt N + N sqrt N), where Q is the size of `queries_` and N is the total size of the interval
// to be considered Mo's algorithm note: answer should accept an inclusive inclusive range
template<typename IndexType, typename AnsType, typename It>
std::vector<AnsType> chunkQueries(
    It begin, It end, const std::function<void(IndexType)> &add,
    ...
    size_t Q = 0;
    for (auto val = begin; val != end; ++val, ++Q)
        queries.push_back({val->first, val->second, Q});
