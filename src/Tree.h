...
constexpr size_t numberOfBits(size_t x) { return x < 2 ? x : 1 + numberOfBits(x >> 1); }
...
template<
    ...
    typename Weight_,
    std::enable_if_t<
        std::is_arithmetic_v<Weight_> && Weight_() == std::numeric_limits<Weight_>::min() &&
            maxV != 0,
        bool> = true>
class Tree {
...
public:
    ...
    struct UndirectedPath {
        ...
        friend bool operator==(const UndirectedPath &a, const UndirectedPath &b) {
            return a.w == b.w && ((a.u == b.u && a.v == b.v) || (a.u == b.v && a.v == b.u));
        }
    ...
    struct Edge : UndirectedPath {
        ...
        Edge flip() const { return Edge(v, u, w); }
...
private:
    ...
    // `jmp[d][node]` is the jmp which is 2^d steps higher than `node`, so `jmp[0][node]` is the
    // direct jmp jump of invalid nodes should not be accessed
    ...
    void buildJumpPtrs(const std::set<Node> &todo) {
        for (Depth d = 0; d < logMaxV; ++d) {
            for (Node node : todo) {
                jmp[d + 1][node] =
                    Edge(jmp[d][jmp[d][node].u].u, node, jmp[d][node].w + jmp[d][jmp[d][node].u].w);
            }
        }
    }
...
public:
    ...
    //             size_t rightSize = findSize(rightChild, currOffset + leftSize + spacing +
    //             repr(node).size() + spacing); return leftSize + spacing + repr(node).size() +
    //             spacing + rightSize;
    //         } else {
    //             assert(false && "Cannot print non binary tree in binary format, use `out << tree`
    //             instead");
    //         }
    //     };
...
private:
    ...
    Display print(Node node) const {
        ...
        d.display.push_front(
            std::string(" ") * (d.offset - nodeData.size() / 2) + nodeData +
...
public:
    ...
    bool isLeaf(Node node) const { return containsNode(node) && children[node].empty(); }
    ...
    size_t E() const { return nodes.size() - 1; }
    // O(1)
    // @returns `E`, the number of edges
    size_t M() const { return nodes.size() - 1; }
    ...
    std::vector<Node> getPathNodes(Edge e) const {
        ...
        for (Node node = e.u; node != mid; node = jmp[0][node].u)
            path.push_back(node);
        path.push_back(mid);

        Node midLen = path.size();
        for (Node node = e.v; node != mid; node = jmp[0][node].u)
            path.push_back(node);
    ...
    std::vector<Edge> getPathEdges(Edge e) const {
        ...
        Node mid = lca(e.u, e.v);
        for (Node node = e.u; node != mid; node = jmp[0][node].u)
            path.push_back(jmp[0][node].flip());

        Node midLen = path.size();
        for (Node node = e.v; node != mid; node = jmp[0][node].u)
            path.push_back(jmp[0][node]);
        reverse(path.begin() + midLen, path.end());
    ...
    // O(maxV)
    // @returns array, where the ith weight is the height of node i from the root, or INVALID_WEIGHT
    // if node i is invalid
    const std::array<Weight, maxV> &getHeightFromRoot() const { return height; }

    // O(maxV)
    // @returns array, where the ith weight is the height of node i from the furthest leaf, or
    // INVALID_WEIGHT if node i is invalid
    ...
    // O(V)
    // Finds whether the tree is a height balanced tree - depth of every left and right subtree
    ...
    // O(V)
    // Finds whether this this is a node balanced tree - whether V is in the range [2^ddepth ..
    // 2*2^depth) Should this still apply to non binary trees?
    ...
    // O(V)
    // @returns Whether this tree is a binary tree - there are at most a left and right child for
    // each node
    bool isBinary() const {
        for (Node node : nodes) {
    ...
    bool isStar() const {
        ...
        return degree(node) == V() - 1 ||
               (degree(node) == 1 && degree(getChildren(node)[0]) == V() - 1);
    }
