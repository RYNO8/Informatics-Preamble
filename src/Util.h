...
template<
    typename U, std::enable_if_t<std::is_integral<U>::value, bool> = true, typename S,
    std::enable_if_t<std::is_integral<S>::value, bool> = true>
...
template<typename T>
std::basic_string<T> repeat(const std::basic_string<T> &input, size_t num) {
...
template<typename T>
std::basic_string<T> operator*(std::basic_string<T> str, std::size_t n) {
...
typename std::iterator_traits<It>::value_type sum(const It &begin, const It &end) {
    return accumulate(begin, end, typename std::iterator_traits<It>::value_type(0));
}
